...
class RnnQNetNG(BaseModel):
    ...
    def _build_model(self, word_embedding, char_embedding, encoder_units_number, attention_size,
        ...
        with tf.variable_scope('word_output_layer',
            ...
            _, self.s1 = tf.nn.bidirectional_dynamic_rnn(inputs=self.s1, cell_fw=fwgru, cell_bw=bwgru, dtype=tf.float32,
                                                         scope='s1')
            ...
            tf.get_variable_scope().reuse_variables()
            _, self.s2 = tf.nn.bidirectional_dynamic_rnn(inputs=self.s2, cell_fw=fwgru, cell_bw=bwgru, dtype=tf.float32,
                                                         scope='s2')
        ...
        with tf.variable_scope('char_output_layer',
            ...
            _, self.s3 = tf.nn.bidirectional_dynamic_rnn(inputs=self.s3, cell_fw=fcgru, cell_bw=bcgru, dtype=tf.float32,
                                                         scope='s3')
            ...
            tf.get_variable_scope().reuse_variables()
            _, self.s4 = tf.nn.bidirectional_dynamic_rnn(inputs=self.s4, cell_fw=fcgru, cell_bw=bcgru, dtype=tf.float32,
                                                         scope='s4')
        ...
        with tf.variable_scope('output_layer', initializer=tf.contrib.layers.xavier_initializer(uniform=True)) as scope:
            ...
            self.o = add_dense_layer(self.o, [256, 128, 64, 2], self.dropout_keep_prob, activation=tf.nn.relu,
                                     use_bias=True)
